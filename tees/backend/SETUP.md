...
3. In the editor's function dropdown pick **`setupSheet`** → Run → grant the
   ...
   colours) and `Stock` (12 seeded design rows) — so you never hand-build
   columns.
...
# test order (expect {"ok":true,"orderId":"PT-TEST-01",...}; a row appears in Orders,
# stock for agent-01/M drops by 1, and you get the email)
curl -sL -X POST '<EXEC_URL>' -H 'Content-Type: text/plain' --data '{
  "type":"order","secret":"<SECRET>","orderId":"PT-TEST-01",
  "designId":"agent-01","size":"M","qty":1,"amountShown":499,
  "buyer":{"name":"Smoke Test","room":"H0-000","phone":"9999999999"},
  "note":"delete me","self":true,"deliveryMode":"hostel","hp":"","t":9999}'
```
...
QR. Then fill real `price` + size counts in the `Stock` tab and go live.
...
2. Paste those SKUs into `Stock` columns `podSkuS…podSkuXXL`.

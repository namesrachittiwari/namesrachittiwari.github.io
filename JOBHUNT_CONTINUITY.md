...
1. **Review console loop**: every substantive agent reply publishes/updates the annotatable review page (URL in §0.3) — and per user mandate 2026-07-17 it must be a VISUAL EXPLAINER: diagrams, flows, wireframe-style blocks with tiny captions; chat replies stay short (a few sentences + the link), never long text walls. THEME FROZEN (v7, user-mandated — do not change without an explicit user request): strict black-and-white only; NO emojis; ONE typeface (system sans) everywhere; ALL text bold (700, emphasis 800); hierarchy via size only; status encoded via border weight/style and text glyphs.
...
## 3c. Claude Design MCP (added 2026-07-17, commit e18b239)
`.mcp.json` at this repo's root registers the Anthropic-hosted design MCP: `https://api.anthropic.com/v1/design/mcp` (verified live: 405 on GET, 401 unauthenticated = OAuth; api.anthropic.com is direct-egress allowlisted, so the restrictive network policy does NOT block it). It loads at session start, not mid-session. Activation: **(a)** next Claude Code session on this repo → approve the project server → authenticate via `/mcp`; **(b) recommended for phone + both conversations:** add as a claude.ai custom connector (Settings → Connectors → Add custom connector → paste the URL) — it then appears in the Claude-app Frontend conversation AND in Claude Code sessions like the other org connectors. Note: this does not fix the meetyoinky fetch blocker (pixels must still come from user screenshots).
